...
public class Yanolja {
	public int max_hotel = 5;
	public int max_pension = 1;
	Notice notice = new Notice();

	public int reservedRoomCount(String accommodationType, String reserve) {
		if (reserve.equals("reserve")) {
			if (accommodationType.equals("호텔") & this.max_hotel > 0) {
				return max_hotel--;

			} else if (accommodationType.equals("펜션") & this.max_pension > 0) {
				return max_pension--;
	...
	public void reserveYanolja(String clientID, TypeStrategy accommodationType, String room_type, boolean luxury_pillow,
		...
		UserFactory userFactory = new YanoljaUserFactory();
		...
		if (reservedRoomCount(userInfo.get("accommodation_type"), "reserve") > 0) {
			String message = userInfo.get("client_id") + " 고객님, " + 
		userInfo.get("accommodation_type") + " 예약되었습니다.\n\n";
			
			printDetails(userInfo, room_type, luxury_pillow, luxury_blanket, break_fast, tf, message);
		} else {
			String message = userInfo.get("client_id") + " 고객님 죄송합니다. " + userInfo.get("accommodation_type") + " 빈 방이 없습니다.\n";
			tf.setText(message);
	...
	public void unreserveYanolja(String clientID, TypeStrategy accommodationType, TextArea tf) {
//		 예약 취소
		UserFactory userFactory = new YanoljaUserFactory();
		User user = userFactory.newInstance();
		Map<String, String> userInfo = user.unreservation(clientID, accommodationType);
		String message = userInfo.get("client_id") + " 고객님, " + 
		userInfo.get("accommodation_type") + " 예약이 취소되었습니다.\n";
		tf.setText(message);
	...
	// additional services
	public HotelRoom getAdditionalHotelService(HotelRoom h, boolean luxury_pillow, 
	...
	public PensionRoom getAdditionalPensionService(PensionRoom p, boolean luxury_pillow, boolean luxury_blanket, boolean break_fast) {
		if (luxury_pillow)
	...
	public void printDetails(Map<String, String> userInfo, String room_type, boolean luxury_pillow, boolean luxury_blanket,
			boolean break_fast, TextArea tf, String message) {
